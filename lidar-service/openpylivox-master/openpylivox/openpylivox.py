...
class _heartbeatThread(object):
    ...
    def run(self):
        """
        looks for incoming data from the lidar unit (and sends heartbeat requests)
        """
        print("Using local openpylivox")
        while True:
            if self.started:
                ...
                if select.select([self.t_socket], [], [], 0.1)[0]:
                    binData, addr = self.t_socket.recvfrom(1500)
                    tempObj = openpylivox()
...
class openpylivox(object):
    ...
    def extrinsicParameters(self):
        if self._isConnected:
            if self._showMessages:
                print("      x: " + str(self._x if self._x is not None else "None") + " m")
                print("      y: " + str(self._y if self._y is not None else "None") + " m")
                print("      z: " + str(self._z if self._z is not None else "None") + " m")
                print("      roll: " + ("{0:.2f}".format(self._roll) if self._roll is not None else "None") + " deg")
                print("      pitch: " + ("{0:.2f}".format(self._pitch) if self._pitch is not None else "None") + " deg")
                print("      yaw: " + ("{0:.2f}".format(self._yaw) if self._yaw is not None else "None") + " deg")
